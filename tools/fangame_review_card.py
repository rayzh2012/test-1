...
def main():
    ...
    ap.add_argument('--smoke',required=False)
    ap.add_argument('--target',required=False)
    ap.add_argument('--out-json',default='fangame_review_card.json')
    ap.add_argument('--out-md',default='fangame_review_card.md')
    a=ap.parse_args()
    st=json.loads(Path(a.static).read_text(encoding='utf-8'))
    sm={}
    ...
    maps=mc.get('maps_loaded',st.get('map_count',0)) or 0
    events=mc.get('event_commands',0) or 0
    dialogue=mc.get('dialogue_lines',0) or 0
    choices=mc.get('choice_options',0) or 0
    commons=mc.get('common_events',0) or 0
    skills=mc.get('skills',0) or 0
    items=mc.get('items',0) or 0
    enemies=mc.get('enemies',0) or 0
    transfers=mc.get('map_transfers',0) or 0
    shops=mc.get('shop_calls',0) or 0
    ...
    # Structural interest is deliberately not called a review of actual gameplay.
    ai_interest=clamp(0.70*content+0.30*play)

    hist=target.get('historical_reputation') or {}
    hist_rating=hist.get('rating_5')
    hist_votes=hist.get('votes')
    hist_downloads=hist.get('downloads')
    hist_note=hist.get('summary')
    if hist_rating is None and target.get('historical_rating_10') is not None:
        hist_rating=float(target['historical_rating_10'])/2.0
    ...
    if status in ('GAMEPLAY_LIKELY','INPUT_RESPONSE_VERIFIED'): verdict='高优先：内容规模强，CI已有真实输入响应'
    elif status=='BOOT_VERIFIED': verdict='值得玩：已验证启动，尚需确认进入实际地图'
    elif st.get('playability_structural')=='STRUCTURAL_OK': verdict='值得保存/继续测：结构完整，但当前运行路径未完全验证'
    else: verdict='低优先：先解决完整性或运行路径问题'

    card={
      'name':target.get('name') or target.get('title') or st.get('archive'),
      'engine':st.get('engine'),
      'evidence_separation':'Historical reputation and AI structural prediction are independent. AI score is NOT a claim that the game was played through.',
      'historical_player_rating_5':hist_rating,
      'historical_votes':hist_votes,
      'historical_downloads':hist_downloads,
      'historical_summary':hist_note,
      'ci_playability_status':status,
      'ci_playability_score_5':round(play,2),
      'content_scale':st.get('content_scale'),
      'metrics':{'maps':maps,'event_commands':events,'dialogue_lines':dialogue,'choice_options':choices,'common_events':commons,'skills':skills,'items':items,'enemies':enemies,'map_transfers':transfers,'shop_calls':shops},
      ...
      'verdict':verdict,
      'confidence':'MEDIUM' if mc.get('maps_loaded',0)>0 else 'LOW_TO_MEDIUM'
    }
