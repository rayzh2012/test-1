...
def write_audit(out, audit):
    (out/'genome_audit.json').write_text(json.dumps({'schema':'fangame-genome-audit-v1.3','stages':audit},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')

def emit_public_report(out, normalized, parser_version, audit):
    ...
    s=run([sys.executable,str(ROOT/'tools/fangame_public_report.py'),str(normalized),
           ...
           '--registry-entry-out',str(registry_entry),'--parser-version',parser_version])
    audit.append({'stage':'public_report',**s}); require(s,'public_report')
    return public_json, public_md, registry_entry
...
def main():
    ...
    if opaque:
        summary={
          'schema':'fangame-genome-runner-v1.5','engine':st.get('engine'),'game_root':st.get('game_root'),
    ...
    if is_mv_family(st.get('engine')):
        ...
        public_json,public_md,registry_entry=emit_public_report(out,normalized,'rpgmaker_mv_genome.v0.1',audit)
        ...
        summary={
          'schema':'fangame-genome-runner-v1.5','engine':n.get('engine') or st.get('engine'),'game_root':st.get('game_root'),
    ...
    normalized=out/'normalized_profile.json'
    norm_cmd=[sys.executable,str(ROOT/'tools/fangame_normalized_profile.py'),
              '--static',str(static),'--package',str(package),'--out',str(normalized),
              '--dialogue-corpus',str(out/'dialogue_corpus.json'),'--map-graph',str(out/'map_graph.json'),
              '--content-inference',str(out/'content_inference.json')]
    if a.target_json: norm_cmd += ['--target-json',str(Path(a.target_json).resolve())]
    s=run(norm_cmd); audit.append({'stage':'normalize_profile',**s}); require(s,'normalize_profile')
    public_json,public_md,registry_entry=emit_public_report(out,normalized,'rpgmaker_rgss_normalized.v0.1',audit)
    ...
    summary={
      ...
      'public_report_status':'EMITTED','public_report_json':public_json.name,'public_report_markdown':public_md.name,
      'public_registry_entry':registry_entry.name,'baseline_status':n.get('baseline_status'),'parser_family':n.get('parser_family'),
      'total_pipeline_wall_time_sec':round(sum(x['wall_time_sec'] for x in audit),3),
      'stage_wall_time_sec':{x['stage']:x['wall_time_sec'] for x in audit},
      'note':'Inspectable legacy RGSS lane now emits the same normalized-profile/public-report contract as MV/MZ. Engine-specific parser family remains explicit; cross-engine percentiles stay disabled until calibrated ordinary-RPG baselines exist.'
    }
    (out/'genome_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    write_audit(out,audit)
    print(json.dumps(summary,ensure_ascii=False,indent=2))
