...
def literal_text_chars(root: Path):
    exts={'.txt','.rb','.json','.ini','.js','.csv','.md'}; chars=cjk=words=0
    for p in root.rglob('*'):
...
def marshal_probe(root: Path, work: Path, engine: str):
    ...
    out=work/'marshal_content.json'; p=subprocess.run(['ruby',str(script),str(root),'--out',str(out)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
    if out.exists():
        try:
            data=json.loads(out.read_text(encoding='utf-8')); data['runner_returncode']=p.returncode; data['runner_log_tail']=p.stdout[-2000:]; return data
        except Exception: pass
...
def main():
    ...
    result={'archive':src.name,'extract_ok':ok,'extract_backend':backend,'extract_attempts':attempts}
    if not ok:
